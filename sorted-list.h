...
/*
 * Structure of tyoe node that holds data and a pointer to the next noe in the linked list
 */
struct SortedListNode
{
    struct SortedListNode *next;
    void *data;		//Holds void data
    int boolean;    //used as a flag to see if a node should be returned or not based on if it has been deleted by the user
    int refCount;   //keeps count of reference pointers that point to the node
};
typedef struct SortedListNode Node;
...
struct SortedList
{
    Node *front;	//pointer to front node of list
    int size;		//size of elements in the list
...
{
    Node *ptr;	//pointer to node to return next
};
